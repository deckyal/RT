...
class MTCNN():
    ...
    def haveFace(self,img, facedetector):
        ...
        #tic()
        boundingboxes, points = self.detect_face(img_matlab, minsize, PNet, RNet, ONet, threshold, False, factor)
        #toc()
    ...
    def doFaceDetection(self,the_image):
        
        img_matlab = the_image.copy()
        tmp = img_matlab[:,:,2].copy()
        img_matlab[:,:,2] = img_matlab[:,:,0]
        img_matlab[:,:,0] = tmp
    
        # check rgb position
        return self.detect_face(img_matlab, self.minsize, self.PNet, self.RNet, self.ONet, self.threshold, False, self.factor)
                        
        '''if len(boundingboxes > 1): 
            max = 0;
            i_max = 0;
            for idx in range(0,len(boundingboxes)): 
                if boundingboxes[idx,4] > max : 
                    max = boundingboxes[idx,4]
                    i_max = idx
            boundingboxes = np.asarray([boundingboxes[i_max]])
        
        if (len(boundingboxes) <= 0 ): #No face 
            err = 99999
            iou = 0
            bbox = [9999,9999,9999,9999]
        else : 
            bbox =boundingboxes[0]
            print("bbox : ",bbox.shape,bbox)
            
            bbox_gt = utils.read_kp_file(list_data[i])
            bbox_gt = utils.get_bb(bbox_gt[:,0],bbox_gt[:,1])
            
            print(bbox,bbox_gt)
            
            #Now displaying
            
            cv2.rectangle(the_image,
